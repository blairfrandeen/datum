...
class TestJson:
    def _empty_measurements(self, fh):
        return {"measurements": [{"goose": "Frank", "feathers": True}]}
    ...
    def _empty_expressions(self, fh):
        return {
            "measurements": [
                {
                    "name": "Frank",
                    "expressions": [{"bbq": False, "answer": 42, "networth": None}],
                }
            ]
        }
    ...
    def test_get_json_key_value_pairs(self, caplog, monkeypatch):
        ...
        assert isclose
        (valid_names["GEARS.mass"], 54.45, 0.001)
        # Test for value with a named component
        assert isclose
        (valid_names["BOUNDING.length.x"], 547.0, 0.001)
        # Test for value from a list
        assert isclose
        (valid_names["HOUSING.first_moments_of_inertia.1"], 120320.589, 0.001)
...
class TestXL:
    mock_source_dict = {"k1": 12, "k2": 3, "k4": [4, 3, 2]}

    def _mock_target_dict(self, arg):
        return {"k1": 15, "k3": 9, "k4": [1, 2, 3]}
    ...
    def test_dump(self, monkeypatch, caplog):
        ...
        monkeypatch.setattr(
            xlpnr, "get_json_key_value_pairs", lambda _: self.mock_source_dict
        )
        xlpnr.dump(self.workbook, "json_test.json")
        assert self.workbook.sheets[sheet_name].range("A1:B1").value == [
            "PARAMETER",
            "VALUE",
        ]
        ...
        monkeypatch.setattr(
            xlpnr, "load_metadata_from_json", lambda _: {"test": "datum"}
        )
        xlpnr.dump(self.workbook, "json_test.json")
        assert self.workbook.sheets[sheet_name].range("A1:B1").value == [
            "test",
        ...
        assert self.workbook.sheets[sheet_name].range("A3:B3").value == [
            "PARAMETER",
            "VALUE",
        ]
        for index, key in enumerate(self.mock_source_dict):
            ...
            assert (
                self.workbook.sheets[sheet_name].range(f"B{index+4}").value
                == test_value
            )
    ...
    def test_get_workbook_kvp(self, caplog):
        ...
        assert valid_workbook["Test_Matrix"] == [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
    ...
    def test_write_named_ranges(self, monkeypatch, capsys, caplog):
        ...
        xlpnr.write_named_ranges(
            self._mock_target_dict, self.mock_source_dict, self.workbook, "test"
        )
        captured = capsys.readouterr()
    ...
    def test_preview_named_range_update(self, monkeypatch, capsys):
        ...
        xlpnr.preview_named_range_update(mock_target_dict, self.mock_source_dict)
        captured = capsys.readouterr()
    ...
    def test_write_named_range(self, monkeypatch, caplog):
        ...
        with pytest.raises(TypeError):
            xlpnr.write_named_range(
                self.workbook, "Test_Single_Value", MockXLName("test")
            )
        ...
        single_test_values = [
            ...
            "good morning!",
            None,
            datetime.datetime(1984, 6, 17),
            None,
        ]
        for value in single_test_values:
            # test that function executes
            assert (
                xlpnr.write_named_range(self.workbook, "Test_Single_Value", value)
                == value
            )
            # independent test to show it wrote correctly
            assert (
                self.workbook.names["Test_Single_Value"].refers_to_range.value == value
            )
        ...
        test_matrices = [  # assume already flattened
            [1, 2, 3, 4, 5, 6, 7, 8, 0],
            [1.1, 2.2, 3.3, 4.4, 5.3, 0.93, 1, None, 0.98],
            [
                "blair",
                "needs",
                "to",
        ...
        for matrix in test_matrices:
            # test that function executes
            assert (
                xlpnr.write_named_range(self.workbook, "Matrix_Test", matrix) == matrix
            )
            # independent test to show it wrote correctly
            # requires manual flattening, only check one value
            assert (
                self.workbook.names["Matrix_Test"].refers_to_range.value[1][1]
                == matrix[4]
            )

        # test writing to a range that's too small
        assert xlpnr.write_named_range(self.workbook, "too_small_range", [1, 2, 3]) == [
            1,
            2,
        ]
        assert xlpnr.write_named_range(
            self.workbook, "too_small_range", [None, None]
        ) == [None, None]
        ...
        assert xlpnr.write_named_range(self.workbook, "too_large_range", [1, 2, 3]) == [
            1,
            2,
            3,
        ]
        assert xlpnr.write_named_range(
            self.workbook, "too_large_range", [None, None, None]
        ) == [None, None, None]
